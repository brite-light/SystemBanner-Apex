...
namespace SystemBanner
{
    partial class Banner
    {
        ...
        {   int bannerWidth = System.Windows.Forms.Screen.AllScreens[bannerScreen].Bounds.Width; //be as wide as the screen it's on
            ...
            // Quick test to remove the Method for Check MouseOver
            ...
            {
                // Disabled mouse-over opacity change
                // if (MouseOver(this))
                // {
                //    this.Opacity = 0.3;
                // }
                // Change 'else if' to 'if' to disable mouse-over opacity change
                if (IsForegroundFullScreen() && Program.bannerTopAndBottom)
                {
                // I updated this setting to .99 from .3 to make it static to the lower opacity.  If we want to make this a variable will need to change.
                    this.Opacity = 0.99;
                    if (bannerPosition)
                    {
                        this.Top = System.Windows.Forms.Screen.AllScreens[bannerScreen].WorkingArea.Bottom - bannerHeight;
                    }
                }
